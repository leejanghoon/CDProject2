...
for i in trackinginfo:
  ...
  if x1<=center_x<=x2 and y1<=bottom_y<=y2 : #우하단의 count boundary를 지나간 차량 갯수 count
    ...
    parked[id] += 1
    count_parked_car = sum(1 for count in parked if count >= 200) # 200프레임이상동안 머물렀다면 주차된 걸로 인식하고 count up
      
  dict[fr]=(count_car,count_truck,count_motorcycle, count_parked_car)

count_vehicle = count_car+count_motorcycle+count_truck
print("총 차량수 : {}, 승용차 : {}, 이륜차 : {}, 화물차 : {}, 주차된 차 : {}".format(count_vehicle, count_car, count_motorcycle, count_truck, count_parked_car))

# 출력
x3,y3,x4,y4=900,10,1280,200 # count 표시
for i in range(length):
  img=cv2.imread("C:/VehicleVideo/4/%d.jpg"%(i+1))
  ...
  cv2.putText(img,'Trcuk',(x3+10,y3+75),0,1,(0,0,0),thickness=1)
