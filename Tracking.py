...
Num = 'vid3' # 트래킹하고 싶은 영상 번호 지정
vid = cv2.VideoCapture('/home/vislab/UAV/dataconversion/bumtracking/video3.mp4') # 원본 영상 경로, 원본 영상 불러오기
f = open('/home/vislab/UAV/dataconversion/bumtracking/vid3convertnew.txt', 'r') # Detection한 좌표 텍스트파일 불러와야함

start = time.time()
os.environ["CUDA_VISIBLE_DEVICES"] = '0,1' # GPU 사용
motdata = '/home/vislab/UAV/dataconversion/bumtracking/v3' # 원본 사진에 대해서 Tracking
print(motdata)
list_motdata = natsort.natsorted(os.listdir(motdata)) # 원본 사진을 프레임 순으로 정렬

########## 아래 코드는 detection한 좌표를 json 파일로 만들어주는 과정(헷갈리면 json 파일을 직접 열어서 확인)
file_data = OrderedDict() # file_data에 bbox, labels, scores를 포함한 딕셔너리를 넣음
lines = f.readlines() # detection한 좌표 텍스트 파일을 읽음
for line in lines:
  line = line.rstrip().split()
  fr = int(line[0]) # index 0이 프레임
  file_name = str(fr) + '.jpg' # {"프레임.jpg" : [{"bbox" : [], "labels" : [], ...}} 형식으로 들어가서 key값인 프레임.jpg 설정
  if file_name not in file_data: # 처음에 프레임.jpg가 없다면 배열을 만들어야함
    file_data[file_name] = []
    ...
    tmp = {"bbox": [float(line[2]), float(line[3]), float(line[4]), float(line[5])], "labels": line[1], "scores": 1} # 위의 형식(bbox, labels ...)에 맞게 딕셔너리 만듬
    file_data[file_name].append(tmp) # file_data에 추가
with open(f'/home/vislab/UAV/dataconversion/bumtracking/{Num}.json', 'w') as make_file: # json 파일 만드는 코드
  json.dump(file_data, make_file, ensure_ascii=False)
##########

matplotlib.use('Agg') 
jsonpath = f'/home/vislab/UAV/dataconversion/bumtracking/{Num}.json'
...
save_path = f'/home/vislab/UAV/dataconversion/bumtracking/result{Num}/' # 영상이 저장될 경로 지정
########## Tracking 결과 영상이 저장될 경로로 이동
os.chdir('/home/vislab/UAV/dataconversion/bumtracking')
os.system(f'rm -r result{Num}')
...
out = cv2.VideoWriter(f'/home/vislab/UAV/dataconversion/bumtracking/result{Num}/result{Num}.avi', codec, vid_fps, (vid_width, vid_height))
##########

########## SORT 알고리즘 동작 코드
from collections import deque
pts = [deque(maxlen = 10155) for _ in range(50000)] # 차들이 이동한 흔적 좌표를 저장
sort = '/home/vislab/UAV/gdrive/MOT/sort/' # SORT 알고리즘의 경로 지정
...
with tf.device("/device:GPU:0"): # GPU를 사용하여 tracking 수행
    ...
    #####
    f2 = open(f"/home/vislab/UAV/dataconversion/bumtracking/result{Num}/Tracking_coordinate.txt", "a") # Tracking 결과 좌표를 저장하기 위해 a로 오픈
    for key in natsort.natsorted(odata.keys()): # odata에 있는 key값들을 불러옴
      ...
      """
      with open(f"/home/vislab/UAV/gdrive/MOT/{Num}_roi.json","r") as f1:
        ploydata=json.load(f1)# 우리가 관심없는 영역을 검은색으로 처리하기 위한 좌표들을 불러옴
      
      polypts=[]
      polyfile=ploydata['shapes']
      for i in polyfile:
        temp=i['points']
        for j in temp:
          newlist=[]
          newlist.append(int(j[0]))
          newlist.append(int(j[1]))
          polypts.append(newlist)
        poly=np.array(polypts)
        cv2.fillPoly(overlay,[poly],(0,0,0))
      f1.close()
      """

      """
      ##### 관심없는 영역을 검은색으로 처리하는 코드
      while True:
        line = f1.readline()
        if not line: break
        x, y, w, h = map(int, line.split())
        overlay[y : y + h, x : x + w] = (0, 0, 0)
      f1.close()
      #####
      """
